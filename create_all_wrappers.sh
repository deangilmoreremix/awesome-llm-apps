...
# Script to create instant_lab wrappers for all apps
...
./find_apps.sh | while read app_dir; do
    ...
    app_name=$(basename "$app_dir" | sed 's/[^a-zA-Z0-9_]/_/g')
    ...
    if [ -f "$app_dir/app.py" ]; then
        ...
        # Find first Python file that looks like main
        main_script=$(find "$app_dir" -maxdepth 1 -name "*.py" | head -1 | xargs basename 2>/dev/null)
    fi
    
    if [ -z "$main_script" ]; then
        echo "ERROR: No main script found for $app_name"
        continue
    fi
